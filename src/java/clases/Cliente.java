...
public class Cliente {
   ...
   public Cliente(String id) {
    if (id == null || id.trim().isEmpty()) {
        throw new IllegalArgumentException("El ID del cliente no puede ser null o vacío.");
    }
    ...
    try (PreparedStatement stmt = ConectorBD.getConnection().prepareStatement(rowSql)) {
        ...
        if (result.next()) {
            ...
            throw new SQLException("No se encontró ningún cliente con el ID: " + id);
        }
    } catch (SQLException ex) {
        Logger.getLogger(Cliente.class.getName()).log(Level.SEVERE, "Error al obtener cliente", ex);
    }
}
    ...
    public TipoIdentificacion getTI() {
        return new TipoIdentificacion(id_tipo_identificacion);
    }
